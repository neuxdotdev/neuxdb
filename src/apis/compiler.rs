use crate::apis::sintax::Statement;
...
pub fn parse(sql: &str) -> Result<Statement> {
    ...
    if parts.is_empty() {
        return Err(NeuxError::Parse("Empty query".into()));
    }
    match parts[0] {
        ...
        _ => Err(NeuxError::Parse(format!("Unknown command: {}", parts[0]))),
    }
}
fn parse_create(parts: &[&str]) -> Result<Statement> {
    if parts.len() < 4 || parts[1] != "table" {
        return Err(NeuxError::Parse(
    ...
    let open = rest
        .find('(')
        .ok_or_else(|| NeuxError::Parse("Missing '('".into()))?;
    let close = rest
        .rfind(')')
        .ok_or_else(|| NeuxError::Parse("Missing ')'".into()))?;
...
fn parse_insert(parts: &[&str], unquote: &dyn Fn(&str) -> String) -> Result<Statement> {
...
fn parse_select(parts: &[&str], unquote: &dyn Fn(&str) -> String) -> Result<Statement> {
    ...
    if parts.len() > 4 && parts[4] == "where" && parts.len() >= 6 {
        ...
        let eq_pos = cond_str
            .find('=')
            .ok_or_else(|| NeuxError::Parse("Invalid condition".into()))?;
...
fn parse_update(parts: &[&str], unquote: &dyn Fn(&str) -> String) -> Result<Statement> {
    ...
    let eq_set = set_pair
        .find('=')
        .ok_or_else(|| NeuxError::Parse("Missing '=' in SET".into()))?;
    ...
    let eq_cond = cond_pair
        .find('=')
        .ok_or_else(|| NeuxError::Parse("Missing '=' in WHERE".into()))?;
...
fn parse_delete(parts: &[&str], unquote: &dyn Fn(&str) -> String) -> Result<Statement> {
    ...
    let eq_pos = cond_pair
        .find('=')
        .ok_or_else(|| NeuxError::Parse("Invalid condition, expected col=value".into()))?;
